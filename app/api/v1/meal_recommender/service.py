...
async def recommend_meal(request: MealRecommendRequest) -> DailyMealRecommendation:
    ...
    user_prompt = (
        ...
        "2) 알러지나 disliked_items에 포함된 재료는 어떤 끼니에도 사용하지 마세요.\n"
        "3) 체중감량은 하루 총 칼로리를 적당히 낮추고 포만감 있게, "
        "   근육량증가는 단백질 비율이 높은 메뉴로, 혈당/콜레스테롤 관리는 "
        "   해당 지표를 악화시키는 재료(단순당, 포화지방 등)를 피해서 구성하세요.\n"
        "4) comment 필드에는 오늘 하루 식단을 이렇게 추천한 이유, 목표 달성에 어떻게 도움이 되는지 "
        "   한국어로 자연스럽게 설명해주세요.\n"
        "5) cautions에는 알러지, 혈당/콜레스테롤 관리 측면에서의 주의사항이나 보완 팁을 "
